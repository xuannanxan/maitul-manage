 ...
 * @LastEditTime: 2020-03-22 20:57:31
 * @LastEditors: Xuannan
 -->
<template functional>
  <a-sub-menu :key="props.menuInfo.id">
    <span slot="title">
      <nuxt-link :to="{path:(props.menuInfo.url?props.menuInfo.url:'article/')+props.menuInfo.id}"><a-icon v-if="props.menuInfo.icon" :type="props.menuInfo.icon" :style="{ fontSize: '16px'}"/>{{props.menuInfo.name}}</nuxt-link>
    </span>
